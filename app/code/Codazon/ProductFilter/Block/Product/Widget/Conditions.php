...
class Conditions extends Template implements RendererInterface
{
    ...
    protected function _construct()
    {
        ...
        }else if ($widgetParameters = $this->getRequest()->getParam('widget')) {
            $widgetParameters = json_decode($widgetParameters,true);
            if(isset($widgetParameters['values'])){
                $widgetParameters = $widgetParameters['values'];
                if(isset($widgetParameters['conditions_encoded'])) {
                    $widgetParameters['conditions'] = $this->conditionsHelper->decode($widgetParameters['conditions_encoded']);
                    $this->rule->loadPost($widgetParameters);
                }
            }
        }
