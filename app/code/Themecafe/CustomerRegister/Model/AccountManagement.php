...
class AccountManagement implements AccountManagementInterface
{
    ...
    public function __construct(
        ...
        \Themecafe\CustomerRegister\Helper\Data $themecafehelper
    ) {
        ...
        $this->themecafehelper = $themecafehelper;
    }
    ...
    public function authenticate($username, $password)
    {
        ...
        try {
            ...
            throw new InvalidEmailOrPasswordException(__('Invalid login ID or password.'));
        }
    ...
    /**
     * Themecafe : Customized method for sending OTP and Reset Password link through SMS
     * {@inheritdoc}
     */
    public function initiatePasswordReset($email, $template, $websiteId = null)
    {
        ...
        /*customize to send sms to mobile number*/
        if($this->themecafehelper->isActive()){
            $objectManager = ObjectManager::getInstance();
            $link = $this->storeManager->getStore()->getBaseUrl()."customer/account/createPassword/id/".$customer->getId()."/token/".$newPasswordToken;
            ...
            $customerSession->setData('forgotpasswordotpcode', $otpcode);
            $customerSession->setData('forgotpasswordtimestamp', $timestamp);
            $customerSession->setData('forgotpasswordmobile_verified',false);
            $customerSession->setData('forgotpasswordthemecafeMobilenumber', "");
            $customerSession->setData('passwordResetLink', $link);
            $customerSession->setData('forgotPasswordEmail', $email);
            
            $customerSession->setData('forgotPasswordContact', "");
            if($customer->getCustomAttribute('themecafe_mobile')){
                $contactnumber = $customer->getCustomAttribute('themecafe_mobile')->getValue();
                if($contactnumber){
                    $customerSession->setData('forgotpasswordthemecafeMobilenumber', $contactnumber);
                    $forgotMessage = $this->themecafehelper->getConfig('themecafe_customer_verification/otp_label_message/forgot_password');
                    $tinylink = file_get_contents("http://tinyurl.com/api-create.php?url=".$link);
                    $message = str_replace("##LINK##", $tinylink,  $forgotMessage);

                    $customerSession->setData('forgotPasswordContact', $contactnumber);
                    $this->sendOTP($contactnumber, $message, $tinylink, $customerSession, $otpcode);
                }
            }
        }
        try {
    ...
    /**
     * Themecafe : Send OTP and Link SMS
     */
     public function sendOTP($contactnumber, $linkmessage, $tinylink, $customerSession, $otpcode) {
        $objectManager = ObjectManager::getInstance();
//        $themecafeHelper = $objectManager->get('Themecafe\CustomerRegister\Helper\Data');
        
        $storeId = $this->storeManager->getStore()->getId();
        
         /*if config field is empty then send link only*/
        if(trim($linkmessage)==""){
            $message = "Reset your SuAyu password OTP is : ".$otpcode." or Reset password link is : ".$tinylink;
        }
        else{
            $message = str_replace("##OTP##", $otpcode,  $linkmessage);
        }
        try {
            $this->themecafehelper->sendSms($contactnumber, $message, 'ALL', $storeId);
            $response = 0;
            if($this->themecafehelper->getQueryStringElement()) {
                $response = 1;
            }
...
//                __('A customer with the same email already exists in an associated website.')
                __('This email id is already registered, try another email id or "login".')
            );
...
//            throw new InvalidEmailOrPasswordException(__('Invalid login or password.'));
    ...
    /**
     *  
     * Themecafe : Send email with reset password confirmation link with OTP code
     *
     * @param CustomerInterface $customer
     * @return $this
     */
    public function sendPasswordResetConfirmationEmail($customer)
    {
        ...
        $customerEmailData = $this->getFullCustomerObject($customer);
        /*themecafe : customized email to send otp with email*/
        if ($this->themecafehelper->isActive()) {
            $objectManager = ObjectManager::getInstance();
            $customerSession = $objectManager->get('Magento\Customer\Model\Session');
            $this->sendEmailTemplate(
                ...
                self::XML_PATH_FORGOT_EMAIL_IDENTITY,
                ['customer' => $customerEmailData, 'store' => $this->storeManager->getStore($storeId), 'OTPCODE' => $customerSession->getData('forgotpasswordotpcode')],
                $storeId
            );
        }
        else{
            $this->sendEmailTemplate(
                ...
                self::XML_PATH_FORGOT_EMAIL_TEMPLATE,
                self::XML_PATH_FORGOT_EMAIL_IDENTITY,
                ['customer' => $customerEmailData, 'store' => $this->storeManager->getStore($storeId), 'OTPCODE' => ""],
                $storeId
            );
        }
